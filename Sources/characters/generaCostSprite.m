function [animatory_states,tileset]=generaCostSprite(carpeta, costid)
...
kk=dir(strcat(carpeta,'\*.png'));
for as=0:(length(kk)-1);
    imname=(kk(as+1).name);
    im=imread(strcat(carpeta,'\',imname));
    im=rgb2gray(im);
    %im=im(1:2:end,1:2:end);
    graphic=im>0;% graphic=not(graphic);
    mask=im==150;
...
genera_datos(tileset,animatory_states,kk, carpeta, costid);
...
function genera_datos(tileset,animatory_states,kk,carpeta,costid)
...
for i=1:length(animatory_states)
        ...
        for k1=1:6+1
         for k2=1:4+1
                fprintf(fid,'%d',animatory_states(i).data(k1,k2)-1);
             if(k2<4+1)
                fprintf(fid,', ');
             end
         end
